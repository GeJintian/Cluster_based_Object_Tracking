...
template <typename PointT>
class MOT: public rclcpp::Node {
public:
    typedef std::vector<pcl::PointCloud<PointT>> Ring;
    typedef std::vector<Ring> Zone;

    MOT(const rclcpp::NodeOptions &options);
    ...
    void object_detection(const pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &cloud_object, builtin_interfaces::msg::Time time);
    Eigen::Vector3d computeNormal(pcl::PointCloud<PointT> &cloud);

private:

    string left_bound_file;
    string right_bound_file;

    // Every private member variable is written with the undescore("_") in its end.
    std::recursive_mutex mutex_;

    double xy_leaf_size_ = 0.33;
    double z_leaf_size_ = 0.1;
    double cluster_min_dist_ = 0.3;
    ...
    bool display_position_ = true; // displays ego and other positions
    bool enable_RNR_ = true;
    bool enable_RVPF_= true;
    bool enable_TGR_= true;

    bool non_ground_pub_ = true;
    bool objects_cloud_pub_ = true;
    bool curbside_pub_ = true;
    ...
    rclcpp::Publisher<a2rl_bs_msgs::msg::Perception>::SharedPtr pub_cluster_;
    ...
    void publish_single_perception(std::vector<LidarClusterProperties> &collections, builtin_interfaces::msg::Time time);
    ...
    void get_lane(pcl::PointCloud<PointT> &cloud);
    Eigen::Vector3d enu2local(double x, double y);
    Eigen::Vector2d local2enu(double x, double y);

    std::shared_ptr<detection_helper::Reference> left_bound_ptr;
    std::shared_ptr<detection_helper::Reference> right_bound_ptr;
    a2rl_bs_msgs::msg::VectornavIns latest_vn_msg_;
    bool is_frozen = false;
    Eigen::Matrix4d transformMatrix;

    float left_s_guess{0.0};
    float right_s_guess{0.0};
...
template<typename PointT> inline
Eigen::Vector3d MOT<PointT>::enu2local(double x, double y){
    Eigen::Vector4d eigen_point(x, y, 0, 1);
    Eigen::Vector4d result = transformMatrix.inverse()*eigen_point;
    Eigen::Vector3d return_value(result(0),result(1),0);
    return return_value;
}
...
void MOT<PointT>::publish_single_perception(std::vector<LidarClusterProperties> &collections, builtin_interfaces::msg::Time time){
    ...
    if(collections.empty()){
        ...
        ST_.track(closest_x, closest_y);
    }
    vector<double> result = ST_.report();
    ...
    if(result.empty()) {
        RCLCPP_INFO(this->get_logger(),"Nothing found. Publish 99");
        center.x = 99;
        center.y = 99;
        close.x = 99;
        close.y = 99;
    ...
    speed.x = 0;
    speed.y = 0;
    timestamp.nanoseconds = static_cast<unsigned long long>(time.sec)*1000000000+static_cast<unsigned long long>(time.nanosec);
...
template<typename PointT> inline
Eigen::Vector3d MOT<PointT>::computeNormal(pcl::PointCloud<PointT> &cloud) {
    ...
    // eig_dx.col(2) = eig_dx.col(0).cross(eig_dx.col(1));  // Optional: Ensure the right-hand rule
    Eigen::Vector3d normal = eig_dx.col(0);  // Column with the smallest eigenvalue    
    return normal;
}
...
void MOT<PointT>::object_detection(const pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &cloud_object, builtin_interfaces::msg::Time time){
    ...
    if (point.z >= Z_LOWER_BOUND && point.z <= Z_UPPER_BOUND && point.y*point.y+point.x*point.x > SELF_BOUND*SELF_BOUND && point.x > -50 && point.x < 50 && point.y > -30 && point.y < 30) {
        ...
        double front_min_angle_rad = -LIDAR_FRONT_ANGLE * (M_PI / 180.0);
        double front_max_angle_rad = LIDAR_FRONT_ANGLE * (M_PI / 180.0);
        double left_min_angle_rad = 1.2;
        double left_max_angle_rad = 2.86;
        double right_min_angle_rad = -3;
        double right_max_angle_rad = -1.281;
        if ((azimuth >= front_min_angle_rad && azimuth <= front_max_angle_rad)
            ||(azimuth >= left_min_angle_rad && azimuth <= left_max_angle_rad)
            ||(azimuth >= right_min_angle_rad && azimuth <= right_max_angle_rad)
            ){
            cloud_temp.push_back(point);
        }
    }
    ...
    for(int idx = 0; idx < lidar_cluster_properties.size(); idx++){
        ...
        if( //true
            ...
            // for norm
            Eigen::Vector3d normal = computeNormal(*(lidar_cluster_properties[idx].merged_cloud_));
            double dotProduct = normal.dot(z_norm);
            double magnitude1 = normal.norm();
            double magnitude2 = z_norm.norm();
            double cosineAngle = dotProduct / (magnitude1 * magnitude2);
            cosineAngle = std::max(std::min(cosineAngle, 1.0), -1.0);
            double angleRadians = std::acos(cosineAngle);
            double angleDegrees = angleRadians * 180.0 / M_PI;
            if (angleDegrees > 90.0) {
                angleDegrees = 180.0 - angleDegrees;
            }
            if(angleDegrees > ANGLE_RANGE)
            {
...
template<typename PointT> inline
void MOT<PointT>::get_lane(pcl::PointCloud<PointT> &cloud){
    detection_helper::FrenetPoint l_sd = left_bound_ptr->sd({transformMatrix(0,3), transformMatrix(1,3)}, left_s_guess-250);
    detection_helper::FrenetPoint r_sd = right_bound_ptr->sd({transformMatrix(0,3), transformMatrix(1,3)}, right_s_guess-250);
    int num = 50;
    // std::vector<detection_helper::FrenetPoint> ls_collect;
    // std::vector<detection_helper::FrenetPoint> rs_collect;
    for(int i = 0; i < num; i++){
        PointT pl, pr;
        Eigen::Vector3d lxy = enu2local(left_bound_ptr->xy({l_sd.s+i,0}).x,left_bound_ptr->xy({l_sd.s+i,0}).y);
        Eigen::Vector3d rxy = enu2local(right_bound_ptr->xy({r_sd.s+i,0}).x,right_bound_ptr->xy({r_sd.s+i,0}).y);
        pl.x = lxy(0);
        pl.y = lxy(1);
        pl.z = lxy(2);
        pl.intensity = 0;
